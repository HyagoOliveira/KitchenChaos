...
namespace Meryel.UnityCodeAssist.Editor
{
    public class FeedbackWindow : EditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            if (errorCount > 0)
                EditorGUILayout.LabelField($"{errorCount} error(s) found in logs. Please submit a feedback (via e-mail or Discord) with the logs if possible.", styleLabel, GUILayout.ExpandWidth(true));
            else if (warningCount > 0)
                EditorGUILayout.LabelField($"{warningCount} warnings(s) found in logs. Please submit a feedback (via e-mail or Discord) with the logs if possible.", styleLabel, GUILayout.ExpandWidth(true));
            else
                EditorGUILayout.LabelField("No errors found in logs. Please submit a feedback (via e-mail or Discord) describing what went wrong or unexpected.", styleLabel, GUILayout.ExpandWidth(true));
